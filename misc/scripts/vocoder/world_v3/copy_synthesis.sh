...
world_v3="${merlin_dir}/tools/bin/WORLD_v3/"
...
then
    ...
    do
        ...
        ### WORLD ANALYSIS -- extract vocoder parameters ###
        $world_v3/worldAnalysis ${wav_dir}/$file_id.wav ${lf0_dir}/$file_id.lf0 ${mgc_dir}/$file_id.mgc ${bap_dir}/$file_id.bap $f0method $mgcsize $bapsize $alpha $f0_floor $f0_ceil

        #if [ "$issynth" -eq 1 ]
        #then
            ### WORLD Re-synthesis -- reconstruction of parameters ###
            #$world_v3/worldSynthesis  ${lf0_dir}/$file_id.lf0 ${mgc_dir}/$file_id.mgc ${bap_dir}/$file_id.bap ${out_dir}/$file_id.resyn.wav $fs $mgcsize $bapsize $alpha $beta $worldmethod
        #fi
        #$world_v3/test ${wav_dir}/$file_id.wav $file_id.resyn.wav
    done
fi
